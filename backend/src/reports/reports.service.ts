...
export class ReportsService {
  ...
  async generateReportData(projectId: string, hours = 72) {
    ...
    const endpoints = Array.from(endpointMap.entries()).map(
      ([endpoint, data]) => {
        ...
        const errorRate = Number(((data.errors / data.count) * 100).toFixed(2));
  ...
  async exportCsv(projectId: string, hours = 72): Promise<string> {
    const { summary, endpoints } = await this.generateReportData(
      projectId,
      hours,
    );
  ...
  async exportPdf(projectId: string, hours = 72): Promise<Buffer> {
    ...
    return new Promise((resolve) => {
      ...
      doc
        ...
        .fillColor('#666666')
        .text(
          `Last ${hours} hours | Generated: ${new Date().toLocaleString()}`,
          { align: 'center' },
        );
      doc.moveDown(1.5);
